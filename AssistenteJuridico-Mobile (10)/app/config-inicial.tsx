...
import { saveConfig, autoDetectProvider } from "@/services/storage";
import { autoDetectProvider as detectAI } from "@/services/ai";
...
export default function ConfigInicial() {
  ...
  const chave1Info = chave1 ? detectAI(chave1) : null;
  const chave2Info = chave2 ? detectAI(chave2) : null;
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {passo === 1 && (
          <View style={styles.card}>
            ...
            <Text style={styles.cardDesc}>Defina uma senha para proteger o app. Você pode pular esta etapa se preferir acesso direto.</Text>
        ...
        {passo === 2 && (
          <View style={styles.card}>
            ...
            <Text style={styles.cardDesc}>Configure suas chaves de API. O app detecta automaticamente o provedor (Groq, Gemini, OpenAI, etc). Sem chave, usa a Demo Groq gratuita.</Text>
            ...
            {chave1Info && <Text style={styles.detected}>✅ Detectado: {chave1Info.nome} — {chave1Info.modelo}</Text>}

            <Text style={styles.label}>Chave Secundária (Chave 2) — opcional</Text>
            ...
            {chave2Info && <Text style={styles.detected}>✅ Detectado: {chave2Info.nome} — {chave2Info.modelo}</Text>}
            ...
            <View style={styles.infoBox}>
              ...
              {[
                ...
                ["sk-or-...", "OpenRouter — modelos variados"],
                ["sk-ant...", "Anthropic Claude"],
                ["pplx-...", "Perplexity (busca web)"],
                ["sk-...", "OpenAI GPT-4o"],
              ].map(([prefix, desc]) => (
                <Text key={prefix} style={styles.infoItem}>• <Text style={styles.infoPrefix}>{prefix}</Text> {desc}</Text>
              ))}
            </View>
          </View>
        )}
        ...
        {passo === 3 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>🗄️ Banco de Dados (opcional)</Text>
            <Text style={styles.cardDesc}>Cole a URL do seu banco Neon (PostgreSQL) para sincronizar dados entre dispositivos. Se deixar vazio, tudo fica salvo localmente no celular.</Text>

            <TextInput
              style={[styles.input, { minHeight: 80 }]}
              placeholder="postgresql://usuario:senha@host.neon.tech/nomedobanco?sslmode=require"
            ...
            <View style={styles.infoBox}>
              ...
              <Text style={styles.infoItem}>2. Crie um projeto → copie a "Connection string"</Text>
              <Text style={styles.infoItem}>3. Cole aqui acima</Text>
              <Text style={[styles.infoItem, { marginTop: 8, color: COLORS.textMuted }]}>
                Sem banco: histórico e snippets ficam salvos localmente. Funciona normalmente.
              </Text>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  infoPrefix: { fontFamily: "monospace", color: COLORS.primary },
