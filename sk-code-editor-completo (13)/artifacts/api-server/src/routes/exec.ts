...
router.post("/projects/:projectId/exec-stream", (req, res) => {
  ...
  // Keepalive: envia comentário SSE a cada 20s pra o proxy do Replit não matar a conexão
  // durante instalações longas onde npm fica silencioso por > 30s
  const keepaliveTimer = setInterval(() => {
    if (finished) { clearInterval(keepaliveTimer); return; }
    try { res.write(":keepalive\n\n"); (res as any).flush?.(); } catch { clearInterval(keepaliveTimer); }
  }, 20_000);
